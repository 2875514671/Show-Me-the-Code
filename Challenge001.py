...
@pysnooper.snoop()
def string_list(raw):
    result = ''.join([chr(((ord(c) + 2) - ord('a')) % 26 + ord('a')) if 'a' <= c <= 'z' else c for c in raw])
    print(result)
    return result
...
# 4. str.maketrans()
@pysnooper.snoop()
def rewrite_maketrans(raw):
    a = "abcdefghijklmnopqrstuvwxyz,. '()"
    b = "cdefghijklmnopqrstuvwxyzab,. '()"
    # c = list(zip(a, b))
    d = dict(zip(a, b))
    result = "".join([d[x] for x in raw])
    print(result)
    return result


if __name__ == '__main__':
    # raw = "g fmnc wms bgblr rpylqjyrc gr zw fylb. rfyrq ufyr amknsrcpq ypc dmp. bmgle grgl zw fylb gq glcddgagclr ylb " \
